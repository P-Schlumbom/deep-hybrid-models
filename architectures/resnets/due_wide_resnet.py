...
class SensitiveWideBasic(nn.Module):
    def __init__(
        self,
        wrapped_conv,
        wrapped_batchnorm,
        ...
        if stride != 1 or in_c != out_c:
            pooling_kernel_size = int((original_input_size + 2 - ((original_input_size + 1) / stride)))
            #self.shortcut = wrapped_conv(input_size, in_c, out_c, 1, stride)
            self.shortcut = nn.Sequential(
                nn.AvgPool2d(pooling_kernel_size, stride=1),
                wrapped_conv(input_size, in_c, out_c, 1, stride=1)
            )
        else:
            self.shortcut = nn.Identity()
...
class DUE_WideResNet(nn.Module):
    def __init__(
        self,
        input_size,
        spectral_conv,
...
class DUE_SensitiveWideResNet(nn.Module):
    def __init__(
        self,
        input_size,
        spectral_conv,
