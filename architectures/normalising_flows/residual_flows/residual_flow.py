...
import architectures.normalising_flows.residual_flows.layers as layers
import architectures.normalising_flows.residual_flows.layers.base as base_layers
...
def standard_laplace_logprob(z):
    """
    laplace pdf = 1/2b * exp(-|x-mu|/b), hence
    log(pdf) = -log(2b) - |x-mu|/b, so
    mu=0,b=1 => -log(2) - |x|
    :param z:
    :return:
    """
    return -math.log(2) - torch.abs(z)
...
class ResidualFlow(nn.Module):
    ...
    def __init__(
        ...
        #self.n_scale = min(len(n_blocks), self._calc_n_scale(input_size))  # huh what the hell is this
        ...
        print(f"scale: {self.n_scale}")
        print(f"fc: {fc}")
        if not self.n_scale > 0 and not fc:
    ...
    def forward(self, x, logpx=0, inverse=False, classify=False):
        """
        made logpx 0 by default because that's what it should be for all intended use-cases here
        :param x:
        :param logpx:
        :param inverse:
        :param classify:
        ...
        #print(torch.min(x).item(), torch.mean(x).item(), torch.max(x).item())
        #print(f"resflow: x input shape = {x.shape}")
        #print(f"len transforms: {len(self.transforms)}")
        out = []
        if classify: class_outs = []
        for idx in range(len(self.transforms)):
            if logpx is not None:
                #print("input:", torch.mean(x).item(), logpx)
                x, logpx = self.transforms[idx].forward(x, logpx)
                #print(torch.mean(x).item(), torch.mean(logpx).item())
            else:
        ...
        if classify:
            ...
            #print(output)
            z, delta_logp = output
            logpz = standard_normal_logprob(z).view(z.size(0), -1).sum(1, keepdim=True)  # standard logporb
            #logpz = standard_laplace_logprob(z).view(z.size(0), -1).sum(1, keepdim=True)
            #logdet = -delta_logp  # probability of x based on 1/|det| = -log(|det|)
            logdet = delta_logp
            #print(f"logdet: {logdet.mean()}")
            #print(f"resflow: z output shape = {z.shape}")
            return logpz, logdet, z
...
def create_resflow(
    ...
    # set default values used by resflow code
    default_values = {
        "n_blocks": n_blocks,
    ...
    # update default values with anything passed into **kwargs
    default_values.update(kwargs)
    # return ResidualFlow model based on these params
    return ResidualFlow(
        input_size,
        **default_values
    )
