...
parser.add_argument("--dataset", type=str, choices=['CIFAR10', 'CIFAR100', 'SVHN', 'stink-bugs', 'stripes'], default='CIFAR10',
...
parser.add_argument("--nf_epochs", default=None, type=int, help="number of extra NF training epochs, None by default")
parser.add_argument("--lr", default=1e-4, type=float, help="learning rate used for nf optimiser")
parser.add_argument("--dnn_lr", default=0.05, type=float, help="DNN (feature extractor) learning rate, 0.05 by default.")
parser.add_argument("--optim", default="sgd", type=str, choices=["adam", "sgd"], help="optimiser for the DNN component")
...
parser.add_argument("--lr_schedule", default="60-120-160", type=str, help="set the schedule for when the learning "
                                                                          ...
                                                                          "25-65-115-180")
parser.add_argument("--lr_drop", default=0.2, type=float, help="How much to drop the learning rate by at scheduled "
                                                               "learning rate drops - default is 0.2.")
...
def prepare_stinkbug_data(size=64, num_workers=6, max_samples=2000):
    data_src = "../Datasets/stink-bugs/data"
    data_ref_src = "../Datasets/stink-bugs/data/species-labels.csv"
    train_transform = transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Resize(size=size),
            transforms.RandomHorizontalFlip(0.5),
            transforms.RandomCrop(size=size, padding=4, padding_mode='reflect'),
            AddUniformNoise(scale=1.0/255.0),  # images should be in 0-255 range
    ...
    trainset = InAndOutLocalDataset(data_src, data_ref_src=data_ref_src, train=True,
                                    in_distribution=True, transform=train_transform, max_samples=max_samples)
    testset = InAndOutLocalDataset(data_src, data_ref_src=data_ref_src, train=False,
                                   in_distribution=True, transform=val_transform, max_samples=max_samples)
    OODset = InAndOutLocalDataset(data_src, data_ref_src=data_ref_src, train=True,
                                  in_distribution=False, train_prop=1.0,
                                  transform=val_transform, max_samples=max_samples)  # use 100% of the OOD samples
    ...
    return trainloader, testloader, OODloader, val_transform


def prepare_CIFAR10_data():
    train_transform = transforms.Compose(
        [transforms.ToTensor(),
         AddUniformNoise(scale=1.0 / 255.0),
    ...
    return trainloader, testloader, val_transform
...
def prepare_CIFAR100_data():
    train_transform = transforms.Compose(
        [transforms.ToTensor(),
         AddUniformNoise(scale=1.0 / 255.0),
         transforms.Normalize(np.array([0.5071, 0.4867, 0.4408]),  # from https://github.com/meliketoy/wide-resnet.pytorch/blob/master/config.py
                              np.array([0.2675, 0.2565, 0.2761])),
         transforms.RandomHorizontalFlip(0.5),
         transforms.RandomCrop(size=32, padding=4, padding_mode='reflect')])
    val_transform = transforms.Compose(
        [transforms.ToTensor(),
         transforms.Normalize(np.array([0.5071, 0.4867, 0.4408]),
                              np.array([0.2675, 0.2565, 0.2761])),
         #transforms.RandomHorizontalFlip(0.5),
         #transforms.RandomCrop(size=32, padding=4, padding_mode='reflect')
         ])
...
def prepare_SVHN_data():
    train_transform = transforms.Compose(  # supposedly, the SVHN dataset didn't use any preprocessing... (in the WRN paper)
        [transforms.ToTensor(),
         AddUniformNoise(scale=1.0 / 255.0),
         #transforms.Normalize(np.array([125.3, 123.0, 113.9]) / 255.0,
         #                     np.array([63.0, 62.1, 66.7]) / 255.0),
...
def prepare_CIFAR10_OOD_data(ID_transform):
    ...
    CIFAR10_dataset = torchvision.datasets.CIFAR10(root='./data', train=False,
                                                     download=True, transform=ID_transform)
    CIFAR10_loader = torch.utils.data.DataLoader(CIFAR10_dataset, batch_size=args.batch,
                                                  shuffle=True, num_workers=6)
    return CIFAR10_loader


def prepare_CIFAR100_OOD_data(ID_transform):
    #transform = transforms.Compose(
    #    [transforms.ToTensor(),
    #     transforms.Normalize(np.array([125.3, 123.0, 113.9]) / 255.0,
    #                          np.array([63.0, 62.1, 66.7]) / 255.0),
...
def prepare_SVHN_OOD_data(ID_transform):
    #transform = transforms.Compose(
    #    [transforms.ToTensor(),
    #     #transforms.Normalize(np.array([125.3, 123.0, 113.9]) / 255.0,
    #     #                     np.array([63.0, 62.1, 66.7]) / 255.0),
    ...
    SVHN_dataset = torchvision.datasets.SVHN(root='./data', split='test',
                                             download=True, transform=ID_transform)
    SVHN_loader = torch.utils.data.DataLoader(SVHN_dataset, batch_size=args.batch,
                                              shuffle=True, num_workers=6)
    return SVHN_loader
...
def train(args, model: DHM_normflows, flow_optimizer, dnn_optimizer, trainloader, testloader, OODloaders, scheduler=None):
    update_l = True
    test_results = None
    best_val_acc = 0
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if args.test_every_epoch:
                ...
                test_results, _ = compute_auroc_scores(model, testloader, OODloaders)
...
def nf_train(args, model: DHM_normflows, flow_optimizer, trainloader, testloader, OODloaders, scheduler=None):
    print("---\nstarting nf-only post-training\n---")
    update_l = True
    test_results = None
    best_val_acc = 0
    best_val_loss = np.Inf
    ...
    # with tqdm(range(args.epochs)) as pbar:
    for epoch in range(args.nf_epochs):
        mean_loss = 0.0
        mean_classifier_loss = 0.0
        mean_flow_loss = 0.0
        mean_acc = 0.0
...
if __name__ == "__main__":
    ...
    if args.dataset == 'stripes':
        ...
        # stink bug dataset
        size = 32
        trainloader, testloader, OODloader, test_transform = prepare_stinkbug_data(size=size, max_samples=5000, num_workers=32)  # should try a few different sizes...
        datashape = (size, size, 3)
        n_classes = trainloader.dataset.num_classes
        CIFAR100_loader = prepare_CIFAR100_OOD_data(test_transform)
        SVHN_loader = prepare_SVHN_OOD_data(test_transform)
        OOD_loaders = [OODloader, CIFAR100_loader, SVHN_loader]
    elif args.dataset == 'CIFAR10':
        print("preparing CIFAR10 dataset...")
        # CIFAR 10 data
        trainloader, testloader, test_transform = prepare_CIFAR10_data()
        CIFAR100_loader = prepare_CIFAR100_OOD_data(test_transform)
        SVHN_loader = prepare_SVHN_OOD_data(test_transform)
        datashape = (32, 32, 3)
        n_classes = 10
        OOD_loaders = [CIFAR100_loader, SVHN_loader]
    elif args.dataset == 'CIFAR100':
        print("preparing CIFAR100 dataset...")
        trainloader, testloader, test_transform = prepare_CIFAR100_data()
        CIFAR10_loader = prepare_CIFAR10_OOD_data(test_transform)
        SVHN_loader = prepare_SVHN_OOD_data(test_transform)
        datashape = (32, 32, 3)
        n_classes = 100
        OOD_loaders = [CIFAR10_loader, SVHN_loader]
    elif args.dataset == 'SVHN':
        print("preparing SVHN dataset...")
        trainloader, testloader, test_transform = prepare_SVHN_data()
        CIFAR10_loader = prepare_CIFAR10_OOD_data(test_transform)
        CIFAR100_loader = prepare_CIFAR100_OOD_data(test_transform)
        datashape = (32, 32, 3)
        n_classes = 10
        OOD_loaders = [CIFAR10_loader, CIFAR100_loader]
    else:
        raise Exception(f"Dataset key not recognised! {args.dataset} doesn't match any available key.")
    ...
    scheduler = MultiStepLR(dnn_optimizer, milestones=schedule, gamma=args.lr_drop)

    final_results = {'val_acc': 0, 'val_loss': 0}
    if args.epochs > 0:
        final_results = train(args, dhm, flow_optimizer, dnn_optimizer, trainloader, testloader, OOD_loaders, scheduler)
    if args.nf_epochs is not None:
        final_results = nf_train(args, dhm, flow_optimizer, trainloader, testloader, OOD_loaders, scheduler)
    ...
    if args.test_model:
        ...
        test_results, histogram = compute_auroc_scores(dhm, testloader, OOD_loaders, return_plot=True)
        print("printing test results...")
        print(test_results)

        #fig, tsne_results, data_table = generate_multidata_tsne_plots(dhm, batch_size=max(args.batch // 2, 1),
        #                                                              return_table=True, plot_classes=False,
        #                                                              max_size=30)
        fig, data_table, gmm_results = compute_embeddings(dhm, testloader, OOD_loaders, batch_size=args.batch,
                                                          return_table=True, return_gmm_results=True, max_size=30, embedding_type='mds')
        if gmm_results is not None:
            test_results["gmm_results"] = gmm_results
        ...
        if args.mode == 'disabled':
            ...
            pass
        if fig:
            wandb.log({'feature embedding plot': fig})
            wandb.log({'logprob histogram': histogram})
            wandb.log({
                'embedding_data': wandb.Table(dataframe=data_table),
                #'tsne_results': tsne_results
            })
            # wandb.log({'tsne_data': wandb.Table(dataframe=data_table), 'test_results': results})
        output_string += f"{test_results['OOD_0_AUROC']}"
